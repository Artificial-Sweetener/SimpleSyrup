...
"""Derive narrow shared coherence bands from Anima token ownership."""
...
class AnimaSelfAttentionCoherenceProfile:
...
class AnimaSelfAttentionCoherencePolicy:
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(radius, bool) or not isinstance(radius, int) or radius < 0:
            raise ValueError("Anima coherence radius must be a non-negative integer.")
        if (
            ...
            raise ValueError("Anima interior global blend must be in the range [0, 1).")
        self._radius = radius
    ...
    def resolve(
        ...
        *,
        ...
        if (
            ...
            raise TypeError("Anima coherence owners must use integer B/Q layout.")
        if height < 1 or width < 1 or int(owners.shape[1]) != height * width:
            raise ValueError("Anima coherence owners must match active H/W geometry.")
        ...
        for distance in range(1, self._radius + 1):
            ...
            blend = torch.where(
                ring,
                blend.new_full((), (self._radius + 1 - distance) / (self._radius + 1)),
                blend,
            )
            shared |= expanded
        uncovered = grid.eq(-1)
...
ANIMA_SELF_ATTENTION_COHERENCE_POLICY = AnimaSelfAttentionCoherencePolicy()
