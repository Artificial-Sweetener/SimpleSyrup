...
"""Resolve projected regional masks into deterministic token ownership."""
...
class RegionalSelfAttentionOwnershipPolicy:
    ...
    def relation(owners: torch.Tensor) -> torch.Tensor:
        ...
        if (
            ...
            raise TypeError("Regional attention owners must use integer B/Q layout.")
    ...
    def _validate_masks(masks: torch.Tensor) -> None:
        ...
        if not isinstance(masks, torch.Tensor):
            raise TypeError("Regional attention ownership masks must be a tensor.")
        if masks.ndim != 3 or any(int(size) < 1 for size in masks.shape):
            raise ValueError(
                "Regional attention ownership masks must use R/B/Q layout."
            )
        if not masks.is_floating_point():
            raise TypeError(
                "Regional attention ownership masks must be floating point."
            )
        if not bool(torch.isfinite(masks).all()) or not bool(
            ((masks >= 0.0) & (masks <= 1.0)).all()
        ):
            raise ValueError(
                "Regional attention ownership masks require finite values in the "
                "inclusive range [0, 1]."
            )


REGIONAL_SELF_ATTENTION_OWNERSHIP_POLICY = RegionalSelfAttentionOwnershipPolicy()
