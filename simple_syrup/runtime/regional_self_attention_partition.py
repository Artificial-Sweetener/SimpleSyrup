...
"""Group exact regional ownership relations into compact attention batches."""
...
class RegionalSelfAttentionPartitionGroup:
...
@dataclass(frozen=True, slots=True)
class RegionalSelfAttentionPartitionPlan:
    """Retain shape-compatible calls and exact per-token query coverage."""

    regional_groups: tuple[RegionalSelfAttentionPartitionGroup, ...]
    global_groups: tuple[RegionalSelfAttentionPartitionGroup, ...]
    ...
    def build(
        ...
        if not torch.equal(coverage, expected):
            raise ValueError("Regional attention partitions have invalid coverage.")
        return cls(
    ...
    def _groups(
        ...
        owners: torch.Tensor,
    ) -> tuple[RegionalSelfAttentionPartitionGroup, ...]:
    ...
    @staticmethod
    def _validate(profile: RegionalSelfAttentionCoherenceProfile) -> None:
        """Require aligned ownership, shared-key, and blend tensors."""

        if not isinstance(profile, RegionalSelfAttentionCoherenceProfile):
            raise TypeError("Attention partition requires a coherence profile.")
        owners = profile.query_owners
        if not isinstance(owners, torch.Tensor):
            raise TypeError("Attention partition owners must be a tensor.")
        if owners.dtype != torch.long:
            raise TypeError("Attention partition owners must be integers.")
        if owners.ndim != 2 or any(int(size) < 1 for size in owners.shape):
            raise ValueError(
                "Attention partition owners must use non-empty B/Q layout."
            )
        if bool((owners < -1).any()):
            raise ValueError("Attention partition owners contain invalid indices.")
        if (
            ...
            raise TypeError("Shared keys must use boolean B/Q layout.")
        ...
        if (
            ...
            raise ValueError("Global blend must use finite B/Q values in [0, 1].")
        if not bool(blend[owners.eq(-1)].eq(1.0).all()):
            raise ValueError("Shared queries must use full global attention.")
