...
def test_manifest_names_focused_tests_explicitly() -> None:
    ...
    assert len(tests) == 32
    assert "tests/regional_generation/anima/test_anima_module_surface.py" in tests
    assert "tests/regional_generation/anima/test_anima_multi_lora_fidelity.py" in tests
    assert "tests/regional_generation/anima/test_anima_composition_phase.py" in tests
    assert (
