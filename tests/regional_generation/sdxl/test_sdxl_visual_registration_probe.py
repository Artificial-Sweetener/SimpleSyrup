...
from .support.sdxl_visual_test_inventory import (
    visual_inventory,
)


def test_required_nodes_cover_all_public_sampler_geometries(tmp_path: Path) -> None:
    """Probe the same live registration surface as the 14-output matrix."""

    required = _required_node_ids("run", visual_inventory(tmp_path))
...
def test_nested_choice_validation_is_layout_independent_and_exact() -> None:
    ...
    metadata: JsonObject = {
        "input": {"required": {"ckpt_name": [[CHECKPOINT_SELECTION], {}]}}
    }

    assert CHECKPOINT_SELECTION in _nested_text_values(metadata)
    _require_choice(metadata, CHECKPOINT_SELECTION)
