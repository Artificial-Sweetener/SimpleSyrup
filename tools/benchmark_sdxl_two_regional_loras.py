...
"""Benchmark the matched warmed SDXL regional execution floor."""
...
from tools.sdxl_regional_lora_performance.matched_floor_runner import (
    run_matched_floor_comparison,
)

LOGGER = logging.getLogger(__name__)
DEFAULT_OUTPUT_ROOT = Path(
    r"<COMFY_ROOT>\benchmark_artifacts\universal-regional-adapter\sdxl-matched-floor"
)


def main(argv: Sequence[str] | None = None) -> int:
    """Load external fixtures and run the four-mode warmed comparison."""
    ...
    try:
        ...
        simultaneous = full_strength_composition_cases(inventory, prompt_set)[2]
        result = run_matched_floor_comparison(
            artifacts,
            inventory=inventory,
            case=simultaneous,
        ...
        LOGGER.exception("Matched-floor SDXL benchmark failed at %s", artifacts.root)
        return 1
    LOGGER.info("Matched-floor SDXL benchmark completed: %s", result)
    return 0
