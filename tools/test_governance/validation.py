        ...
        if not path.is_file() or path.parent != root / policy.test_root:
            diagnostics.append(
                Diagnostic(
                    "TPOLICY003",
                    "governance/testing/policy.toml",
                    f"allowed root source {allowed_path} must exist directly "
                    "under the test root",
                )
            )
    return diagnostics
    ...
    for candidate in candidates:
        ...
        diagnostics.append(
            Diagnostic(
                candidate.rule,
                candidate.path,
                f"{candidate.evidence} at {candidate.locator}; perform "
                "source-level review and record an exact classification or "
                "debt-remediation waiver",
            )
        )
    return diagnostics
    ...
    if len(linked) != 1:
        ...
        diagnostics.append(
            Diagnostic(
                "TDEBT006",
                registry,
                f"debt {debt.identifier} and its remediation waiver must cover "
                "identical state",
            )
        )
    return diagnostics
    ...
    if len(waiver.rationale) < _MINIMUM_RATIONALE_LENGTH:
        diagnostics.append(
            Diagnostic(
                "TWAIVER005",
                registry,
                f"waiver {waiver.identifier} requires a substantive "
    ...
    if paths != expected_paths:
        diagnostics.append(
            Diagnostic(
                f"{prefix}003",
                registry,
                "record paths must exactly equal sorted candidate paths "
                f"{expected_paths}",
            )
        )
    elif (
