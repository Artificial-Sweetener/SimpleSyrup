...
TYPESCRIPT_LAYOUT_RULE = "TSLAYOUT001"
TYPESCRIPT_OPTIONAL_RULE = "TSOPTIONAL001"

_TYPESCRIPT_OPTIONAL_PATTERN = re.compile(
    r"\b(?:describe|it|test)\.(?:only|skip|todo)\s*\("
)
...
def discover_test_candidates(
    ...
    candidates = [
        ...
        *_typescript_test_candidates(root),
...
def _typescript_test_candidates(root: Path) -> list[TestCandidate]:
    """Find misplaced or optional frontend tests in the authored test tree."""

    test_root = root / "web/tests"
    if not test_root.is_dir():
        return []
    candidates = [
        TestCandidate(
            rule=TYPESCRIPT_LAYOUT_RULE,
            path=path.relative_to(root).as_posix(),
            locator="frontend-root-source",
            evidence="authored source is stored directly under web/tests/",
            line=1,
        )
    ...
    for path in sorted(test_root.rglob("*.ts")):
        for line_number, line in enumerate(
            ...
            if _TYPESCRIPT_OPTIONAL_PATTERN.search(line):
                candidates.append(
                    TestCandidate(
                        rule=TYPESCRIPT_OPTIONAL_RULE,
                        path=path.relative_to(root).as_posix(),
                        locator=f"optional-proof:{line_number}",
                        evidence="frontend proof uses an optional test declaration",
                        line=line_number,
                    )
                )
    return candidates
