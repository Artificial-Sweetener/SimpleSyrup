"""Run the P6.9 tiled public Attention Coupling matrix in isolated ComfyUI."""
...
DEFAULT_OUTPUT_ROOT = Path(
    r"<COMFY_ROOT>\benchmark_artifacts\anima-regional-prompting-v1\p6.9"
)
...
def execute_matrix(
    ...
    *,
    ...
    """Execute every tiled case and return the terminal result path."""
    ...
    recorder = TiledIntegrationResultRecorder(artifacts.root)
    ...
    with ManagedComfyServer(
        ...
        for case, workflow in zip(definitions, workflows, strict=True):
            LOGGER.info("P6.9 starting %s — %s", case.case_id, case.label)
            ...
            if workflow.source_save_node_id is None:
                raise RuntimeError("P6.9 upscale workflow is missing its source image.")
            ...
            LOGGER.info("P6.9 artifact %s — %s", case.label, image_path)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=[],
        help="Run one matrix case; repeat for an ordered diagnostic subset.",
    )
    ...
    try:
        ...
        LOGGER.exception("P6.9 tiled integration failed at %s", artifacts.root)
        return 1
    LOGGER.info("P6.9 tiled integration completed at %s", result)
    return 0
