...
def load_policy(path: Path) -> ArchitecturePolicy:
    ...
    return ArchitecturePolicy(
        ...
        source_files=tuple(
            Path(value) for value in _string_list(structure, "source_files")
        ),
        source_extensions=frozenset(_strings(structure, "source_extensions")),
        excluded_paths=frozenset(_string_list(structure, "excluded_paths")),
...
def _string_list(data: Mapping[str, object], key: str) -> tuple[str, ...]:
    """Return one required array of unique nonempty strings."""
