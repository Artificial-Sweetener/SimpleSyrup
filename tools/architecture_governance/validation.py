        ...
        if len(valid_paths) != len(debt.paths):
            diagnostics.append(
                Diagnostic(
                    "DEBT001",
                    registry,
                    f"debt {debt.identifier} must reference exact governed "
                    "source paths",
                )
            )
            ...
            diagnostics.append(
                Diagnostic(
                    "DEBT002",
                    registry,
                    f"debt {debt.identifier} no longer matches assessed source; "
                    "reassess current responsibilities and the next extraction or "
                    "delete resolved debt",
                )
            )
        if debt.review_by < today:
            diagnostics.append(
                Diagnostic(
                    "DEBT003",
                    registry,
                    f"debt {debt.identifier} review deadline expired on "
                    f"{debt.review_by.isoformat()}",
                )
            )
...
def _validate_unique_state(state: ArchitectureState) -> list[Diagnostic]:
    ...
    for _justification, count in Counter(structural_justifications).items():
        if count > 1:
            diagnostics.append(
                Diagnostic(
                    "WAIVER011",
    ...
    if lines <= policy.hard_lines:
        diagnostics.append(
            Diagnostic(
                "WAIVER005",
                registry,
                f"waiver {waiver.identifier} matches no current hard-size "
                "finding; delete it",
            )
        )
    if lines != waiver.max_lines:
        ...
        if debt is None or waiver.path not in debt.paths:
            diagnostics.append(
                Diagnostic(
                    "WAIVER006",
                    registry,
                    f"waiver {waiver.identifier} must link assessed debt for the "
                    "same exact path",
                )
            )
        if waiver.next_limit is None or waiver.next_limit >= waiver.max_lines:
            diagnostics.append(
                Diagnostic(
                    "WAIVER007",
                    registry,
                    f"waiver {waiver.identifier} next_limit must be lower than "
                    "max_lines",
                )
            )
    else:
        if any(waiver.path in debt.paths for debt in debts.values()):
            diagnostics.append(
                Diagnostic(
                    "WAIVER009",
                    registry,
                    f"structural waiver {waiver.identifier} cannot cover a path "
                    "recorded as mixed-responsibility debt",
                )
            )
        if (
            ...
            diagnostics.append(
                Diagnostic(
                    ...
                    "file-specific cohesion rationale; human source review is "
                    "mandatory",
                )
            )
    return diagnostics
