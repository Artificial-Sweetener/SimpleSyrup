...
"""Run the bounded two-region full-strength SDXL character-LoRA matrix."""
...
DEFAULT_OUTPUT_ROOT = Path(
    r"<COMFY_ROOT>\benchmark_artifacts\universal-regional-adapter\full-strength-composition"
)


def main(argv: Sequence[str] | None = None) -> int:
    """Load external controls and execute exactly three labeled artifacts."""
    ...
    try:
        ...
        LOGGER.exception("Full-strength composition failed at %s", artifacts.root)
        return 1
    LOGGER.info("Full-strength composition completed: %s", result)
    return 0
