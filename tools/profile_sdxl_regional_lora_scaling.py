# SimpleSyrup - workflow-focused ComfyUI extensions for image generation
# Copyright (C) 2026  Artificial Sweetener and contributors
# SPDX-License-Identifier: AGPL-3.0-or-later

"""Profile indexed zero-use and one-use SDXL regional denoiser calls."""

from __future__ import annotations

import argparse
import logging
from collections.abc import Sequence
from pathlib import Path

from tools.comfy_integration.artifacts import IntegrationArtifacts
from tools.sdxl_attention_coupling_integration.visual_inventory import (
    SdxlVisualInventory,
)
from tools.sdxl_attention_coupling_integration.visual_prompt_fixture import (
    load_visual_prompt_set,
)
from tools.sdxl_regional_lora_performance.scaling_profile_runner import (
    run_scaling_profiles,
)

LOGGER = logging.getLogger(__name__)
DEFAULT_OUTPUT_ROOT = Path(
    r"<COMFY_ROOT>\benchmark_artifacts\universal-regional-adapter\sdxl-active-use-profile"
)


def main(argv: Sequence[str] | None = None) -> int:
    """Load external fixtures and capture the bounded profile pair."""

    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--inventory", type=Path, required=True)
    parser.add_argument("--prompt-case", type=Path, required=True)
    parser.add_argument("--comfy-root", type=Path, default=Path(r"<COMFY_ROOT>"))
    parser.add_argument("--output-root", type=Path, default=DEFAULT_OUTPUT_ROOT)
    parser.add_argument("--call-index", type=_positive_call_index, default=30)
    args = parser.parse_args(argv)
    logging.basicConfig(level=logging.INFO, format="%(levelname)s %(message)s")
    artifacts = IntegrationArtifacts(args.output_root)
    try:
        result = run_scaling_profiles(
            artifacts,
            inventory=SdxlVisualInventory.load(args.inventory),
            prompts=load_visual_prompt_set(args.prompt_case),
            comfy_root=args.comfy_root,
            call_index=args.call_index,
        )
    except BaseException as error:
        artifacts.record_failure(error)
        LOGGER.exception("SDXL active-use profiling failed at %s", artifacts.root)
        return 1
    LOGGER.info("SDXL active-use profiling completed: %s", result)
    return 0


def _positive_call_index(value: str) -> int:
    """Parse one positive denoiser call index for bounded profiling."""

    try:
        parsed = int(value)
    except ValueError as error:
        raise argparse.ArgumentTypeError("call index must be an integer") from error
    if parsed < 1:
        raise argparse.ArgumentTypeError("call index must be positive")
    return parsed


if __name__ == "__main__":
    raise SystemExit(main())
