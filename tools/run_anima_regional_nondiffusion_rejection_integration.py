"""Run the P9.6 non-diffusion rejection matrix in one managed ComfyUI."""
...
DEFAULT_OUTPUT_ROOT = Path(
    r"<COMFY_ROOT>\benchmark_artifacts\anima-regional-prompting-v1\p9.6"
)
...
def execute_matrix(
    ...
    *,
    ...
    """Execute the complete ordered rejection matrix and return its result."""
    ...
    mask_writer = MaskArtifactWriter(
        comfy_root / "input",
        f"p96-{artifacts.run_id.lower()}",
    )
    mask_names = mask_writer.write_case(manifest_case, width=WIDTH, height=HEIGHT)
    recorder = AnimaNondiffusionRejectionResultRecorder(artifacts.root)
    ...
    try:
        with ManagedComfyServer(
            ...
            for case, workflow in zip(definitions, workflows, strict=True):
                LOGGER.info("P9.6 starting %s — %s", case.case_id, case.label)
                ...
                LOGGER.info("P9.6 accepted %s — %s", case.label, rejection_path)
    ...
    if process is None:
        raise RuntimeError("P9.6 managed Comfy did not reach ready state.")
...
def _remove_owned_masks(input_root: Path, names: tuple[str, ...]) -> None:
    ...
    for name in names:
        ...
        if not path.is_relative_to(resolved_root):
            raise ValueError("P9.6 mask cleanup escaped the Comfy input root.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        LOGGER.exception("P9.6 integration failed at %s", artifacts.root)
        return 1
    LOGGER.info("P9.6 integration completed at %s", result)
    return 0
