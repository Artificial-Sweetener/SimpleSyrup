"""Validate and persist complete labeled P5.9 public-node evidence."""
...
from .matrix import IntegrationCase


class IntegrationResultRecorder:
    """Own case association, validation, and durable evidence persistence."""
    ...
    def __init__(self, root: Path) -> None:
        ...
        if not self._root.is_dir():
            raise ValueError("P5.9 result root must already exist.")
    ...
    def record_case(
        ...
        *,
        ...
        """Validate metrics and persist one decoded image with exact sidecars."""

        if any(item.get("case_id") == case.case_id for item in self._observations):
            raise ValueError(f"P5.9 case was already recorded: {case.case_id}")
        metrics = self._metrics(history, workflow)
        if wall_runtime_ms <= 0:
            raise ValueError("P5.9 wall runtime must be positive.")
    ...
    def finalize(
        self,
        definitions: tuple[IntegrationCase, ...],
        *,
        ...
        if observed != expected:
            raise ValueError(f"P5.9 observations must match case order: {observed!r}.")
        if not cleanup_verified:
            raise ValueError("P5.9 result requires managed-server cleanup proof.")
    ...
    def _metrics(
        ...
        if not isinstance(outputs, dict):
            raise ValueError("P5.9 history is missing outputs.")
        output = outputs.get(workflow.metrics_node_id)
        if not isinstance(output, dict):
            raise ValueError("P5.9 history is missing metrics output.")
        ...
        if (
            ...
            raise ValueError("P5.9 history must contain one metrics object.")
        metrics = cast(JsonObject, items[0])
        if metrics.get("run_id") != workflow.metrics_run_id:
            raise ValueError("P5.9 metrics run identity does not match its workflow.")
        if metrics.get("model_call_count") != STEPS:
            raise ValueError("P5.9 must execute one complete denoiser call per step.")
        runtime = metrics.get("runtime_ms")
        peak = metrics.get("peak_vram_bytes")
        if not isinstance(runtime, (int, float)) or runtime <= 0:
            raise ValueError("P5.9 measured runtime must be positive.")
        if not isinstance(peak, int) or peak <= 0:
            raise ValueError("P5.9 peak VRAM must be positive.")
        return metrics
    ...
    def _persist(self, *, status: str, extra: JsonObject | None = None) -> Path:
        """Atomically replace the authoritative P5.9 result record."""

        path = self._root / "p5.9-result.json"
        payload: JsonObject = {
            "schema_version": 1,
            "status": status,
            "sampling": {
