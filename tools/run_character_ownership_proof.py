...
"""Run one fixed-seed Character regional-composition proof in managed ComfyUI."""
...
SOURCE_ROOT = (
    ...
    / "benchmark_artifacts"
    / "anima-regional-prompting-v1"
    / "user-prompt-character-character-lora-pair"
...
def main() -> int:
    ...
    try:
        ...
        LOGGER.exception("Character ownership proof failed at %s", artifacts.root)
        return 1
    finally:
        for path in mask_paths:
            path.unlink(missing_ok=True)
    LOGGER.info("Character ownership proof completed at %s", artifacts.root)
    return 0


def _source_prompt() -> dict[str, JsonObject]:
    """Load the exact prior failed Character workflow as the fixed-seed source."""

    decoded = json.loads(
        (SOURCE_ROOT / "character-lora-right-only__workflow.json").read_text(
            encoding="utf-8"
        )
    )
    if not isinstance(decoded, dict):
        raise TypeError("Character source workflow must be a JSON object.")
...
def _write_masks(run_id: str) -> tuple[Path, Path]:
    ...
    paths = (
        COMFY_ROOT / "input" / f"{run_id.lower()}__character-region-00.png",
        COMFY_ROOT / "input" / f"{run_id.lower()}__character-region-01.png",
    )
...
def _specialize(
    ...
    prompt["5"]["inputs"] = {
        "model": ["2", 0],
        "run_id": f"{run_id}:character-ownership:metrics",
    }
    prompt["6"]["inputs"] = {
        "model": ["5", 0],
        "run_id": f"{run_id}:character-ownership:regional-diagnostics",
    }
    prompt["8"]["inputs"] = {
        "latent": ["7", 0],
        "run_id": f"{run_id}:character-ownership:metrics",
    ...
    prompt["11"]["inputs"] = {
        "filename_prefix": (
            f"simple_syrup_character_ownership/{run_id}/character-right-only"
        ),
        "images": ["10", 0],
    }
...
def _label(root: Path) -> None:
    ...
    before = Image.open(
        SOURCE_ROOT / "character-lora-right-only__seed-3141592653.png"
    ).convert("RGB")
    after = Image.open(root / "baseline.png").convert("RGB")
    if before.size != (1024, 1024) or after.size != (1024, 1024):
        raise ValueError(
            "Character proof images must retain original 1024-square resolution."
        )
    header = 96
    ...
    canvas.save(root / "character-ownership__labeled-comparison.png", format="PNG")
